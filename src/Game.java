...
public class Game {
    private static int flood_count = 0;
    private static int flood_optimized_count = 0;
    public GUI theView;
    ...
    public String TITLE = "Flood It Project";
    ...
    public Game(Board board) {
        this(true, board.copy());
    }

    public Game(boolean interactive, Board board) {
    ...
    private void init(Board board) {
        this.board = board;
        currentStep = 0;
        if (interactive) theView = new GUI(this);
    ...
    private void batchTest() {
        ...
        for (int k = 0; k != 2; ++k) {
            ...
            System.out.print("running with " + name + " as the flood function");
            List<Double> timings = new LinkedList<>();
            int prev_size = 0;
            for (int tiles = Constants.DEFAULT_SIZE;
                 tiles <= Constants.MAX_BOARD_SIZE_FOR_AUTOPLAY;
                 tiles += Constants.SIZE_INC_FOR_AUTOPLAY) {
                int size = (int) Math.sqrt(tiles);
                System.out.print("\rPlaying Game: " + (Constants.MAX_BOARD_SIZE_FOR_AUTOPLAY - tiles) / (Constants.SIZE_INC_FOR_AUTOPLAY));
                if (size != prev_size) {
                    prev_size = size;
                    System.out.print("\rtesting a board with " + size + "*" + size + " tiles");
                    if (k == 0)
                        numTiles.add(size * size);
                    resize(size);
                    autoPlay(k, timings);  // use the kth flood function
                    if (k == 0) flood_count += Flood.flood;
                    else if (k == 1) flood_optimized_count += Flood.flood1;
                }
            }
            System.out.println("Total Flood tiles evaluated " + flood_count);
            System.out.println("Total optimized Flood tiles evaluated " + flood_optimized_count);
