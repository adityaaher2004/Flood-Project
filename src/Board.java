...
public class Board {
    ...
    public Board(int size, Tile[][] colors, LinkedList<Coord> flooded) {
    ...
    public Board copy() {
        ...
        WaterColor[][] set_colors = new WaterColor[size][size];

        for (int i = 0; i < tiles.length; i++) {
            Tile[] curr = tiles[i];
            for (int j = 0; j < curr.length; j++) {
                set_colors[i][j] = curr[j].getColor();
            }
        }

        int new_size = this.size;

        return new Board(new_size, set_colors);
